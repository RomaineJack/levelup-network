...
export default function EditArticlePage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchArticle = async () => {
      ...
      if (data) {
        ...
        setCategory((data.categories as any)?.name || '');
      }
      setFetching(false);
    };
    fetchArticle();
  ...
  if (fetching) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '50vh' }}>
      <p style={{ color: '#6b6b8a' }}>Loading article...</p>
    </div>
  );
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '32px 24px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 24 }}>
        <Link href="/admin/articles" style={{ color: '#6b6b8a', textDecoration: 'none', fontSize: 14 }}>← Back</Link>
        <div>
          <h1 className="font-display" style={{ fontSize: 32, fontWeight: 700, color: '#fff', letterSpacing: '.04em' }}>EDIT ARTICLE</h1>
          <p style={{ color: '#6b6b8a', fontSize: 13, fontFamily: 'monospace', marginTop: 2 }}>{slug}</p>
        </div>
      </div>
...
To embed a YouTube video click the ▶ YouTube button above.`}
                rows={18}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <Link href={`/articles/${slug}`} target="_blank" style={{ padding: '10px', background: '#111120', border: '1px solid #252540', borderRadius: 8, color: '#6b6b8a', textDecoration: 'none', fontSize: 13, textAlign: 'center', display: 'block' }}>
            👁 View on Site
          </Link>
        </div>
      </div>
